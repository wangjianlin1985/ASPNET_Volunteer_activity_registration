...
namespace DAL
{
    ...
    public class dalUserInfo
    {
        ...
        {
            string sql = "insert into UserInfo(user_name,password,name,gender,birthDate,userPhoto,telephone,email,address,jifen,regTime) values(@user_name,@password,@name,@gender,@birthDate,@userPhoto,@telephone,@email,@address,@jifen,@regTime)";
            /*构建sql参数*/
            ...
            parm[9].Value = userInfo.jifen; //积分
        ...
        public static ENTITY.UserInfo getSomeUserInfo(string user_name)
        {
            ...
            if (DataRead.Read())
            {
                ...
                userInfo.jifen = float.Parse(DataRead["jifen"].ToString());
        ...
        {
            string sql = "update UserInfo set password=@password,name=@name,gender=@gender,birthDate=@birthDate,userPhoto=@userPhoto,telephone=@telephone,email=@email,address=@address,jifen=@jifen,regTime=@regTime where user_name=@user_name";
            /*构建sql参数信息*/
            SqlParameter[] parm = new SqlParameter[] {
             new SqlParameter("@password",SqlDbType.VarChar),
             new SqlParameter("@name",SqlDbType.VarChar),
